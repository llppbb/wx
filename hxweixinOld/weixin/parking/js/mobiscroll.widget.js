    ...
    ms.classes.Widget = function (el, settings, inherit) {
        ...
        function onBtnEnd(ev) {
            ...
            //点击完毕
             ...
             var hour=$("#test").val().slice(-2);
             var minute=$("#test").val().slice(-5,-3);
             var duration=1;
             $.each($("#numlist").find(".dw-i"),function(i,k){
             	if($(this).hasClass("dw-on")){
             		duration=i+1;
             	}
             })
        ...
        */
        that.show = function (prevAnim, prevFocus) {
            ...
            html += that._generateContent();
             ...
             var html_ = '<div class="dwfl" style="min-width:50px;">'+
			'<div class="dwwlnew slidepics">'+
				'<div class="dwl">分</div>'+
				'<div>'+
					'<div class="dww" style="height:150px;">'+
						'<div class="dw-ulme" style="margin-top: 60px;">'+
							'<div id="numlist">'+
								'<div role="option" aria-selected="false" class="dw-li dw-v" data-val="7" style="height:30px;line-height:30px;">'+
									'<div class="dw-i dw-on">1</div>'+
								'</div>'+'<div role="option" aria-selected="false" class="dw-li dw-v" data-val="8" style="height:30px;line-height:30px;">'+
									'<div class="dw-i">2</div>'+
								'</div>'+'<div role="option" aria-selected="false" class="dw-li dw-v" data-val="9" style="height:30px;line-height:30px;">'+
									'<div class="dw-i">3</div>'+
								'</div>'+'<div role="option" aria-selected="false" class="dw-li dw-v" data-val="1" style="height:30px;line-height:30px;">'+
									'<div class="dw-i">4</div>'+
								'</div>'+'<div role="option" aria-selected="false" class="dw-li dw-v" data-val="2" style="height:30px;line-height:30px;">'+
									'<div class="dw-i">5</div>'+
								'</div>'+'<div role="option" aria-selected="false" class="dw-li dw-v" data-val="3" style="height:30px;line-height:30px;">'+
					...
					'<div class="dwwo"></div>'+
				'</div>'+
				'<div class="dwwol" style="height:30px;margin-top:-16px;"></div>'+
			'</div>'+
		  '</div>';
                       
           setTimeout(function(){
       	     $(".dwwc").eq(0).hide();
       	     $(".dwwc").eq(1).append(html_);
             var startY="";
             var moveY=0;
       	     $(".dwwlnew").on("touchstart",function(event){
       	     	    event.preventDefault();
       	     	    startY=getCoord(event, 'Y');       	     	    
       	     }).on("touchmove",function(event){
       	     	var H=$(this).find(".dw-ulme").attr("H")==undefined?"0":$(this).find(".dw-ulme").attr("H");
       	     	moveY=(getCoord(event,'Y')-startY)+parseInt(H);
       	     	if(parseInt(-moveY)>parseInt(150)){
       	     		moveY=-148;
       	     	}else if(parseInt(-moveY)<0){
       	     		moveY=2;
       	     	}
	       	    $(this).find(".dw-ulme").css({
	       	     		'transition': '-webkit-transform 0.079s ease-out',
	       	     		'transform': 'translate3d(0px, '+moveY+'px, 0px)'
	       	     	})
       	     	
       	     	$(this).find(".dw-ulme").attr("H",moveY);
       	     	...
       	     	if(parseInt(-moveY)>parseInt(150)){
       	     		...
       	     		index_=0;
       	     		console.log(1)
       	     	}else if(parseInt(-moveY)<parseInt(150) && parseInt(-moveY)>0){
       	     		var arr=[0,30,60,90,120,150];//123456
       	     		
			           for(var i=0;i<arr.length;i++){
			           	      if(parseInt(arr[i])<=parseInt(-moveY) && parseInt(-moveY)<parseInt(arr[i+1])){			           	      	
			           	      	var centennum=(arr[i]+arr[i+1])/2;
			           	      	if(parseInt(-moveY)<parseInt(centennum)){
			           	      		moveY=-arr[i];			           	      					           	      		
			           	      	}else{
			           	      		moveY=-arr[i+1];			           	      		
			           	      	}
			           	      	if(parseInt(-moveY)==30){
			           	      		index_=1;
			           	      	}else if(parseInt(-moveY)==60){
			           	      		index_=2;
			           	      	}else if(parseInt(-moveY)==90){
			           	      		index_=3;
			           	      	}else if(parseInt(-moveY)==120){
			           	      		index_=4;
			           	      	}else if(parseInt(-moveY)==150){
			           	      		index_=5;
			           	      	}
		           	      	    break;			           	      	
			           	      }			           	      
			    ...
			    console.log('index_index_index_::'+index_);
			    $("#numlist").find(".dw-li").eq(index_).find(".dw-i").addClass("dw-on").parent(".dw-li").siblings().find(".dw-i").removeClass("dw-on");
       	     	$(this).find(".dw-ulme").css({
       	     		 'transition': '-webkit-transform 0.079s ease-out',
       	     		 'transform': 'translate3d(0px, '+moveY+'px, 0px)'
       	     	})        	     
            ...
            if (hasButtons) {
                ...
                html +='<div class="" style="text-align:center;"><div tabindex="0" role="button" class="dwb">预约时间和时长(小时)</div></div>';
                html += '</div>';
            }
