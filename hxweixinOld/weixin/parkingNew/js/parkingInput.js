...
var next = 0;
var hiHtml='<div style="display:flex;align-items: center;-webkit-align-items: center;justify-content:space-between;-webkit-justify-content:space-between;line-height:.34rem;background:#F6F6F6;color:#D9AA18;font-size:.16rem;border-bottom:1px solid #E0E0E0"><div style="margin-left:.14rem;">输入车牌号</div><img src="weixin/parkingNew/picnew//icon_logo.png" style="height:.14rem;margin-right:.1rem" /></div>';
function showProvince() {
    ...
    $("#pro").html(hiHtml+"<ul class='clearfix ul_pro'>" + ss + "<li class='li_close' onclick='closePro();'><span>关闭</span></li><li class='li_clean' onclick='cleanPro();'><span>清空</span></li></ul>");
    setTimeout(function(){
...
function showKeybord() {
    ...
    $("#pro").html(hiHtml+"<ul class='clearfix ul_keybord'>" + sss + "</ul>");
}
...
function choosekey(obj, jj) {
    // jj= 29 (点击 ok )  jj=37 （点击删除）
    if (jj == 29) {
        // console.log("车牌："+$(".car_input").attr("data-pai"));
        layer.closeAll();
    } else if (jj == 37) {
        if ($(".ppHas").length == 0) {
        ...
        for (var i = 0; i < $(".input_pp").length; i++) {
            ...
            $(".input_pp:eq(" + next + ")").find("span").text($(obj).text().trim());
