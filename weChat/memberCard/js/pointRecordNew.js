...
$(".mobile").html(userData.mobile);
var pointStatusInit = $(".tabListShow").attr("pointStatus");
$("#tab" + pointStatusInit).addClass('tabbotListOn').siblings().removeClass('tabbotListOn');
$(".tabbotListOn").find(".checkedImg").show().parent().siblings().find(".checkedImg").hide();
// 点击分类显示所有的 分类
$(".tabListShow").on("click", function() {
    var that = $(this);
    var tabListShowStatus = that.attr("status");
    $(".tabListBox").css("top", bodyH + "px");
    if (tabListShowStatus == "open") {
        var pointStatus = that.attr("pointStatus");
        console.log($("#tab" + pointStatus))
        $("#tab" + pointStatus).addClass('tabbotListOn').siblings().removeClass('tabbotListOn');
        $(".tabbotListOn").find(".checkedImg").show().parent().siblings().find(".checkedImg").hide();
        $(".mengcengNew").show();
        $('body').css("overflow", "hidden");
        $(".tabListBox").slideDown('100', function() {
            that.attr("status", "close");
        });
        $(".listBox").css({
            "min-height": "100%"
        });
        $("#tabListShowImg").attr("src","pic/icon_dropdow.png");
    } else if (tabListShowStatus == "close") {
        $(".tabListBox").slideUp('100', function() {
            $(".mengcengNew").hide();
            $('body').css("overflow", "auto");
            that.attr("status", "open");
            $(".listBox").css({
                "min-height": "auto"
            });
        });
        $("#tabListShowImg").attr("src","pic/icon_takeback.png");
    }
})
// 点击切换  tab
$(".tabbotList").on("click", function() {
    ...
    //  console.log(index);
    $(".tabListShow").attr("pointStatus", type);
    $(".borbot").css({
        "left": borw * index + "px"
    });
    $(".nodata").hide();
    ...
    $('body').css("overflow", "auto");
    $(".tabListBox").slideUp('100', function() {
        $(".tabListShow").attr("status", "open")
    });
    $("#tabListShowImg").attr("src","pic/icon_takeback.png");
    $(this).addClass('tabbotListOn').siblings().removeClass('tabbotListOn');
    $(".tabbotListOn").find(".checkedImg").show().parent().siblings().find(".checkedImg").hide();
    var tabListShowFont = "";
    if (type == "all") {
        tabListShowFont = "全部积分";
    } else if (type == "daily") {
        tabListShowFont = "日常积分";
    } else if (type == "game") {
        tabListShowFont = "游戏积分";
    } else if (type == "community") {
        tabListShowFont = "社区积分";
    }
    $("#tabListShowFont").html(tabListShowFont);
...
var share = {
    ...
    "shareLink": WX_URL + "weChat/memberCard/pointRecordNew.html",
